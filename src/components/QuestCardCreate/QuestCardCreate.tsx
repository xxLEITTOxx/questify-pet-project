...
export default function QuestCardCreate({ closeForm, type }: Props) {
  ...
  const mutation = useMutation({
    mutationFn: (newCard: CreateCardPayload) => cardService.createCard(newCard),
    onSuccess: () => {
        ...
        <Form className={css.cardContainer}>
          <div className={css.cardHeader}>
            <div className={css.cardHeaderSelector}>
              <Field as="select" name="difficulty" className={css.levelTitle}>
                {DIFFICULTIES.map((d) => (
          ...
          <div className={css.inputContainer}>
            <div className={css.cardTitle}>CREATE NEW {type.toUpperCase()}</div>
            <Field
              type="text"
              name="title"
          ...
          <div className={css.cardBottomContainer}>
            <Field as="select" name="category" className={css.categorySelector}>
              {CATEGORIES.map((c) => (
